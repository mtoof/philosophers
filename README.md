...
If you want to test my program on Linux be aware to change the %llu flag in all printf to %lu, The reason is macOS used the LP64 data model, where long and pointers are 64-bit but on the other hand, Linux typically uses the ILP32 or LP32 data model, where int, long, and pointers are all 32-bit.
